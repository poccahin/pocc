...
整个 Life++ 生态被严格封装为 7 层协议栈，确保物理法则、密码学与人类伦理的绝对解耦与协同。

* **L7 | Human–Agent Interaction Layer** (碳硅交互视界)
* **L6 | Agent Application Layer** (智能体应用生态)
* **L5 | Agent Economy Layer** (宏观硅基经济调控)
* **L4 | Identity & Reputation Layer** (全网身份与 $S_{cog}$ 信用)
* **L3 | Settlement Layer** (超高频价值轧账)
* **L2 | Agent Collaboration Network** (语义意图与坎陷共识)
* **L1 | Edge Compute Network** (主动哈希与空间折叠路由)
* **L0 | Physical Hardware Layer** (裸金属与热力学防线)
...
* **核心模块**: `agent-node`, `agent-router`, `agent-scheduler`, `agent-gossip`。
...
只需三行指令，唤醒你的边缘节点并接入 AHIN 网络：

```bash
# 1. 克隆文明协议栈
git clone https://github.com/lifeplusplus/ahin
cd ahin

# 2. 编译并启动边缘智能体网关
cargo run --release --bin agent-node

# 3. 注册节点，通过密码学熵生成 DID、Wallet 与 Agent ID
ahin register --device-class mac-mini-m4 --stake-usdc 10
```
